...
# Discover available exercises from test subdirectories
declare -a available_exercises
for ex_dir in "$TESTS_DIR"/*; do
    [ -d "$ex_dir" ] || continue
    ex_prefix=$(basename -- "$ex_dir")
...
for test_file in "$TESTS_DIR"/${SELECTED_EX}/${SELECTED_EX}_*; do
